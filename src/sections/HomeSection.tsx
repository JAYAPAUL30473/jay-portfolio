...
export function HomeSection() {
  return <section id="home"><div className="mx-auto grid min-h-[calc(100vh-74px)] w-[min(1160px,calc(100%-32px))] items-center gap-12 py-16 md:grid-cols-[1.1fr_.9fr]">
    <motion.div initial={{ opacity: 0, x: -25 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: .6 }}>
      ...
      <div className="mt-8 flex flex-wrap gap-3"><a className="button-primary" href="#projects">View my work <ArrowDown size={17} /></a><a className="button-secondary" href="/JAY.pdf" download>Download resume <Download size={17} /></a></div>
    ...
    <motion.div className="relative mx-auto grid h-[390px] w-[300px] place-items-center sm:h-[460px] sm:w-[380px]" initial={{ opacity: 0, scale: .9 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: .7 }}><div className="absolute h-[86%] w-[86%] animate-[spin_28s_linear_infinite] rounded-full border border-dashed border-cyan-400/40" /><div className="h-[71%] w-[71%] overflow-hidden rounded-full border-2 border-cyan-400 p-2 shadow-[0_0_50px_rgba(6,182,212,.25)]"><img className="h-full w-full rounded-full object-cover object-[50%_25%]" src="/img/jay3.jpg" alt="Sakhamuri Jayapaul Naidu" /></div><FloatingCard className="right-0 top-10" icon={<Brain size={22} />} title="AI & ML" subtitle="Engineer" /><FloatingCard className="bottom-10 left-0" icon={<Code2 size={22} />} title="Python" subtitle="Developer" /></motion.div>
  </div><Stats /></section>
}
